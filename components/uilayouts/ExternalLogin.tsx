...
export default function ExternalLogin({ provider, buttonText, onClick }: ExternalProps) {
    const label = buttonText ?? `${provider.charAt(0).toUpperCase() + provider.slice(1)}`;
    return (
        <div className="externalLogin flex-1">
            <button className="externalLoginButton" type="button" aria-label={label} onClick={onClick}>
                {/* Service icon */}
